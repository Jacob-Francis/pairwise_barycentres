...
def test_class_with_same_grid_uniform_density(n1, n2, L, grid_type):
    ...
    if isinstance(X, tuple):
        expected_density = torch.ones(
            (X[0].shape[0], X[1].shape[0]), dtype=torch.double
        )
...
def test_class_with_same_grid_shared_density(n1, n2, L, grid_type):
    ...
    shared_density = torch.abs(torch.rand(n1 * n2)) + 1e-5
    shared_density /= shared_density.sum()
    ...
    # check densities are unform
    if grid_type == "tensor" or grid_type == "tuple":
        expected_density = shared_density.view(n1, n2).to(dtype=torch.double)
    else:
        expected_density = shared_density.to(dtype=torch.double).view(n1 * n2, 1)

    for nodes in G.nodes():
        density = bcp.data_dict[nodes]["density"]
        assert np.isclose(density.sum().item(), 1.0)
        # assert density is shared_density
        print(density, expected_density)
        assert torch.isclose(density, expected_density, atol=1e-12, rtol=1e-12).all()

    for edge in G.edges():
        density_i = bcp.data_dict[edge[0]]["density"]
        density_j = bcp.data_dict[edge[1]]["density"]

        assert density_i is density_j
...
def test_class_with_meshgrid_uniform_density(int_list, L, graph_type):
    ...
    # Grid is flat and we're using pykeops so should just add a grid variable
    bcp = BarycentreDataProcessor(
        graph=G, data_dict=data_dict, cuda_device="cpu", free_grids=False
    )
    ...
    for edges in G.edges():
        ...
        dists_x = torch.cdist(grid_i[:n1, 0], grid_j[:m1, 0]) ** 2 / 2.0
        dists_y = torch.cdist(grid_i[0, :n2], grid_j[0, :m2]) ** 2 / 2.0
...
def test_class_with_flat_uniform_density(int_list, L, graph_type):
    ...
    # Grid is flat and we're using pykeops so should just add a grid variable
    bcp = BarycentreDataProcessor(graph=G, data_dict=data_dict, cuda_device="cpu")
...
def test_class_with_pairwise_graph():
...
def test_class_with_same_grid_uniform_density_disconnected(n1, n2, L, grid_type):
