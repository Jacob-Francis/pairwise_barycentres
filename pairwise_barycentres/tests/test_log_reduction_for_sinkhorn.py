...
# For debiasing = TRUE the reduction is the same since we no longer attach the dual potential to the kernal.
# @pytest.mark.parametrize(
#     "n1, n2, members, m1, m2, L, grid_type",
#     [
#         (11, 10, 3, 5, 7, 0.9, "flat"),
...
#         )
#     K = np.exp((Fi.reshape(-1,1) - 0.5 * D) / epsilon.item())
#     expected = np.log(K.T @d.view(-1,1).detach().cpu().numpy() / np.prod(data_processor.data_dict[0]['f'].shape))
...
#     # ---------- expected (NumPy) ----------
#     K = np.exp((Fi.reshape(1,-1) - 0.5 * D) / epsilon.item())
#     expected = np.log(K.sum(1).reshape(-1,1) * d.view(-1,1).detach().cpu().numpy() / np.prod(data_processor.data_dict[1]['f'].shape))
