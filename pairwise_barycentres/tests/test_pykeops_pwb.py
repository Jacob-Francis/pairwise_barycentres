...
def test_log_reduction_ij_matches_numpy(n_i, n_j, d):
    ...
    aj = np.random.rand(n_j).astype(np.float64)
    Fi = np.random.rand(n_i).astype(np.float64)
...
def test_edge_cases_small_values():
    ...
    expected_marginals = (
        ai[:, None] * (np.exp((-0.5 * cost_const * D) / epsilon) * bj[None, :])
    ...
    np.testing.assert_allclose(
        np.asarray(out_r).ravel(), expected_reduction.ravel(), rtol=1e-6, atol=1e-8
    )
    np.testing.assert_allclose(
        np.asarray(out_m).ravel(), expected_marginals.ravel(), rtol=1e-6, atol=1e-8
    )
