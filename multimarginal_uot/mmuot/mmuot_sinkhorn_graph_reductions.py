...
from .mmuot_costing import mmuot_marginal_j, mmuot_marginals, mmuot_dual_cost
...
def generate_mmuotdataprocessor_star_graph(
    ...
    for i in range(M):
        ...
        data_dict[counter] = {
            "density": data[i][
                0
            ],  # this is the bayrcentre which will have a uniform density to start
    ...
    for edge in dp.graph.edges:
        dp.data_dict.setdefault(edge, {})["alpha"] = torch.ones_like(
            dp.data_dict[edge[0]]["f"]
        )
        dp.data_dict.setdefault((edge[1], edge[0]), {})["alpha"] = torch.ones_like(
            dp.data_dict[edge[1]]["f"]
        )
...
def sinkhorn_update(
    dp: SinkhornDataProcessor, j, epsilon, rho, aprox="balanced", prod=True
):
    """
        ...
        for p_j, j in dfs_edges:
            ...
            if j != v0:
                dp.data_dict[(j, p_j)]["alpha"] = alpha_reduction(
                    dp, j, p_j, epsilon, prod=prod
                )

            # Sinkhorn update (including aprox relaxation)
            dp.data_dict[j]["f"], er = sinkhorn_update(
                dp, j, epsilon, rho, aprox=aprox, prod=prod
            )
